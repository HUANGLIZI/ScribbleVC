...
if __name__ == '__main__':
    ...
    parser.add_argument("--arch", default='scribble_ACDC_gradCAM', type=str)
    ...
    parser.add_argument("--tblog", default='tblog', type=str)
    ...
    parser.add_argument('--exp', type=str,
                        default='Prostate/gradCAM', help='experiment_name')
    ...
    parser.add_argument('--weight_pseudo_loss', type=float, default=0.5, help='pseudo label loss')
    ...
    parser.add_argument("--val_every_epoches", default=5, type=int)
    ...
    model = getattr(importlib.import_module(args.network), 'Net_' + args.arch)(linear_layer=args.linear_layer,
    ...
    avg_meter = pyutils.AverageMeter('loss', 'loss_ce', 'loss_pseudo', 'loss_cam', 'loss_cam0', 'loss_cam1', 'loss_cam2', 'loss_cam3',
                                     'loss_crf', 'loss_cls')
    train_dice = 0
    train_accuracy = MultilabelAccuracy(num_labels=num_classes-1).cuda()
    total_rep_count = np.array([0, 0])
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, sampled_batch in enumerate(trainloader):
            ...
            pred1, pred2, cls_output, rep_count = model(img, ep=ep, model_type = "train") \
                                            if not args.no_class_rep else model(img, 0)
            total_rep_count += np.array(rep_count)
            ...
            if not args.val_mode:
                tblogger.add_scalar('rep_count/conv', total_rep_count[0], ep)
                tblogger.add_scalar('rep_count/tran', total_rep_count[1], ep)
                total_rep_count = np.array([0, 0])
            ...
            if (ep + 1) % args.val_every_epoches == 0:
                ...
                total_val_count = np.array([0, 0])
                total_val_acc = 0
                metric_list = []
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i, val_count, val_acc = test_single_volume(
                        sampled_batch["image"], sampled_batch["label"], model, classes=num_classes,
                        patch_size=args.patch_size, epoch=ep,
                        model_type = 'val' if not args.no_class_rep else None,
                        case=sampled_batch["idx"][0])
                    metric_list.append(metric_i)
                    total_val_count = total_val_count + val_count
                    total_val_acc += val_acc
                tblogger_valid.add_scalar('rep_count/conv', total_val_count[0], ep)
                tblogger_valid.add_scalar('rep_count/tran', total_val_count[1], ep)
                total_val_count = np.array([0, 0])
                total_val_acc = total_val_acc / len(valloader)
                print(f"val Accuracy on epoch {ep}: {total_val_acc}")
                tblogger_valid.add_scalar('metric/acc', total_val_acc, ep)
                total_val_acc = 0
                ...
                if performance > best_performance:
                    ...
                    print('best model in epoch %5d  mean_dice : %.4f  PZ : %.4f   CG : %.4f' % (ep, performance, metric_list[0, 0], metric_list[1, 0]))
