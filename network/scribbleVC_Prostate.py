...
class Decoder(nn.Module):
    ...
    def forward(self, feature):
        x0 = feature[0]         # [4, 256, 64, 64]
        x1 = feature[1]         # [4, 512, 32, 32]
        x2 = feature[2]         # [4, 1024, 16, 16]
        x3 = feature[3]
        x4 = feature[4]
...
class Net(nn.Module):
    ...
    def forward(self, x, ep=0, model_type = "train"):   # [6, 1, 256, 256]
        ...
        conv_rep_count, tran_rep_count = 0, 0
        ...
        if ep > 4:  # when dice > 0.5
            if model_type == 'train' and x.shape[0] == self.batch_size:
                ...
                conv_rep_count = torch.sum(torch.where(conv_cls_rep_weight > 0, 1, 0)).item()
                ...
                tran_rep_count = torch.sum(torch.where(tran_cls_rep_weight > 0, 1, 0)).item()

            elif model_type == 'val':
                # (1) add history conv class representation to conv feature representation
                conv_cls_prob = conv_cls.sigmoid()
                conv_cls_rep_weight = torch.where(conv_cls_prob < 0.5, torch.zeros_like(conv_cls_prob), conv_cls_prob) * torch.tensor([0.7,0.3]).cuda()  # 对这个类预测的概率值(<0.5的p 为0)
                x = x + torch.einsum('ab,bjkm->ajkm', conv_cls_rep_weight, self.hist_conv_cls_rep)
        ...
        return seg_conv, seg_trans, [conv_cls,tran_cls], [conv_rep_count, tran_rep_count]